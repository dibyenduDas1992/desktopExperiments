			...
			Modal:{
				...
				Notice:{
					...
					_onOpen:function(){
						...
						function(e,t){
							t=jQuery(t);
							if(t.attr("id")==this.id||t.css(this.options.attributes.y)=="auto"||t.css(this.options.attributes.x)=="auto")
								return;
							if(!this.options.stack){
								t.data("jBox").close({ignoreDelay:true});
								return
							}
							t.css("margin-"+this.options.attributes.y,parseInt(t.css("margin-"+this.options.attributes.y))+this.dimensions.y+10)
						}.bind(this));
						if(this.options.audio&&!this.IE8){
							...
							this.audio[0].play()}}.bind(this),
							_onCloseComplete:function(){this.destroy()}.bind(this)},
							Image:{target:jQuery(window),
							fixed:true,
							blockScroll:true,
							closeOnEsc:true,
							closeOnClick:"overlay",
							...
							_onInit:function(){
								...
								this.imageZIndex=1;
								jQuery.each(this.attachedElements,function(e,t){
									t=jQuery(t);
									if(t.data("jBox-image-gallery"))
										return;
									var n=t.attr(this.options.gallery)||"default";
									!this.images[n]&&(this.images[n]=[]);
									this.images[n].push({src:t.attr(this.options.src),
									label:t.attr(this.options.imageLabel)||""});
									this.options.imageLabel=="title"&&t.removeAttr("title");
									t.data("jBox-image-gallery",n);
									t.data("jBox-image-id",this.images[n].length-1)}.bind(this));
									var e=function(e,t,n,r){
										if(jQuery("#jBox-image-"+e+"-"+t).length)return;
										var i=jQuery("<div/>",{id:"jBox-image-"+e+"-"+t,"class":"jBox-image-container"}).css({backgroundImage:"url("+this.images[e][t].src+")",backgroundSize:this.options.imageSize,
										opacity:r?1:0,
										zIndex:n?0:this.imageZIndex++}).appendTo(this.content);
										var s=jQuery("<div/>",{id:"jBox-image-label-"+e+"-"+t,"class":"jBox-image-label"+(r?" active":"")}).html(this.images[e][t].label).appendTo(this.imageLabel);
										!r&&!n&&i.animate({opacity:1},
										this.options.imageFade)}.bind(this);
										var t=function(e,t){jQuery(".jBox-image-label.active").removeClass("active");
										jQuery("#jBox-image-label-"+e+"-"+t).addClass("active")};
										this.showImage=function(n){
											if(n!="open"){
												var r=this.currentImage.gallery;
												var i=this.currentImage.id+(1*(n=="prev")?-1:1);
												i=i>this.images[r].length-1?0:i<0?this.images[r].length-1:i
											}else{
												var r=this.source.data("jBox-image-gallery");
												var i=this.source.data("jBox-image-id");
												jQuery(".jBox-image-pointer-prev, .jBox-image-pointer-next").css({display:this.images[r].length>1?"block":"none"})
											}
											this.currentImage={gallery:r,id:i};
											if(jQuery("#jBox-image-"+r+"-"+i).length){
												jQuery("#jBox-image-"+r+"-"+i).css({zIndex:this.imageZIndex++,opacity:0}).animate({opacity:1},
												n=="open"?0:this.options.imageFade);
												t(r,i)
											}else{
													this.wrapper.addClass("jBox-loading");
													var s=jQuery('<img src="'+this.images[r][i].src+'">').load(function(){
																															e(r,i,false);
																															t(r,i);
												...
												var o=i+1;
												o=o>this.images[r].length-1?0:o<0?this.images[r].length-1:o;
												!jQuery("#jBox-image-"+r+"-"+o).length&&jQuery('<img src="'+this.images[r][o].src+'">').load(function(){
																																							e(r,o,true)
																																						})
											}
							}
